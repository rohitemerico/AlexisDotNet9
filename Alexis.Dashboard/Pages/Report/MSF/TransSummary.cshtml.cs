...
public class TransSummaryModel(IHttpContextAccessor httpContextAccessor) : BasePageModel(httpContextAccessor)
{
    private MSFController msfController = new MSFController();
    const string FilterType = nameof(AuditCategory.MDM_iOS);
    public string ExportFileName { get => $"{FilterType}_TransactionSummary_{DateTime.Now.ToString("yyyyMMdd")}"; }
    ...
    public List<MSFDepositProductViewModel> TransSummary { get; set; }
    ...
    public override void OnPageHandlerExecuting(PageHandlerExecutingContext context)
    {
        ...
        checkAuthorization(ModuleLogAction.View_FinancingProductMSF);
    }
    ...
    private void BindDataByDateRange(DateTime? minDate, DateTime? maxDate)
    {
        ...
        else
        {
            ...
            data = msfController.GetTransactionSummary(StartDate, EndDate);
            TransSummary = Common.DataHelper.ConvertDataTableToList<MSFDepositProductViewModel>(data);
        }
    }
}
