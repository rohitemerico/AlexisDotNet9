...
public class MSFController : CommonController
{
    //all transaction types
    public List<string> GetAllTransactionTypes()
    {
        var query = @"SELECT DISTINCT TransDesc FROM tbl_app_msf_translog";
        sql.Clear();
        MyParams.Clear();
    ...
    //all transactions
    public DataTable GetTransactionDetails(DateTime minDate, DateTime maxDate)
    {
        var query = @"
                    SELECT 
                    t.TransStatus, t.TransDate, t.TransDesc, t.UserName , t.TblMachineMachineSerial , b.bDesc 
                    FROM   tbl_app_msf_translog AS t
                    ...
                    WHERE t.TransDate >= @p__linq__0 AND t.TransDate <= @p__linq__1 
    ...
    //transaction summary (group by TransDesc, UserName, TblMachineMachineSerial)
    public DataTable GetTransactionSummary(DateTime minDate, DateTime maxDate)
    {
        var query = @" 
                                SELECT TransDesc ,UserName, TblMachineMachineSerial, 
                                COUNT(*) TotalTrans, 
                                ...
                                FROM tbl_app_msf_translog
                                WHERE TransDate >= @p__linq__0 AND TransDate <= @p__linq__1
                                GROUP BY TransDesc, UserName, TblMachineMachineSerial
                            ";
        sql.Clear();
        MyParams.Clear();
        MyParams.Add(new Params("@p__linq__0", "DATETIME", minDate));
    ...
    //transaction summary (group by MSF.TransDesc, Device.MachineName) mainly for dashboard, return jSON result
    ...
    {
        var query = @" 
                                SELECT MSF.TransDesc, Device.MachineName, 
                                COUNT(*) TotalTrans, 
                                COUNT(CASE WHEN TransStatus = 0 THEN 1 ELSE NULL END) AS TotalFailed, 
                                COUNT(CASE WHEN TransStatus = 1 THEN 1 ELSE NULL END) AS TotalPassed
                                FROM tbl_app_msf_translog MSF
                                LEFT JOIN tblMachine Device on Device.MachineSerial = MSF.TblMachineMachineSerial
                                WHERE MSF.TransDate BETWEEN (GETDATE() - @p__linq__0) AND GETDATE()
                                GROUP BY MSF.TransDesc, Device.MachineName ORDER BY MSF.TransDesc, Device.MachineName
                            ";
        sql.Clear();
        MyParams.Clear();
